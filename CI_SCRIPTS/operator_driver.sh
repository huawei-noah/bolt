...
if [[ ${exe_on_device}  == true ]] ; then
    ...
    adb -s ${device} push ${exe_host_path} ${exe_device_path} > /dev/null || exit 1
fi
        ...
        if [[ $params_len -gt 0 ]]; then
            ...
            if [[ ${exe_on_device} == true ]] ; then
                ...
                if [[ ${use_static_library} != true ]] ; then
                    library_reference="export LD_LIBRARY_PATH=${device_dir} &&"
                fi
                adb -s ${device} shell "${library_reference} taskset ${cpu_mask} ${exe_device_path} ${params} || echo '[FAILURE]'" &> status.txt
                ...
                if [[ ${use_static_library} != true ]] ; then
                    export LD_LIBRARY_PATH=${exe_host_path}/../lib:${LD_LIBRARY_PATH}
                fi
                taskset ${cpu_mask} ${exe_host_path} ${params} || exit 1
            fi
        fi
    fi
}

...
cmake_options=""
TEMP=`getopt -o "ht:c:" -al target:,threads:,help,converter:,example,debug,profile,shared,gpu,openmp,flow,serial:,fp32:,fp16:,int8:,train,clean -- "$@"`
if [[ $? != 0 ]]; then
...
while true ; do
    case "$1" in
        ...
        --int8)
            ...
            shift 2 ;;
        --train)
            cmake_options="${cmake_options} -DUSE_TRAINING=ON"
            shift ;;
        --clean)
            clean="on"
            shift ;;

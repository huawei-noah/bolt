...
class Tensorflow2Caffe:
    ...
    def extract_dense(self, input_name, output_name, scope_id, scope_name="dense"):
    ...
    def extract_denses(self, input_name, output_names, output_nums, scope_id, scope_name="dense"):
        if (isinstance(scope_name, str)):
            layer_names = [scope_name, "kernel", "bias"]
        elif (isinstance(scope_name, list)):
            layer_names = scope_name
    ...
    def add_reduce_mean(self, input_name, axis, keep_dim, output_name):
        ...
        self.caffe_model.add_layer(layer)
        self.data_dict[output_name] = Operators.reduction(self.data_dict[input_name], operation, axis, output_name)
        return output_name
    ...
    def extract_lstm(self, input_name, state_name, output_name, scope_id,
            ...
            use_proj=False, zoneout_cell=0, zoneout_output=0):
        if (isinstance(scope_name, str)):
    ...
    def add_pad(self, input_name, output_name, padding_shapes, padding_values=None):
        layer = caffe_net.LayerParameter(name=output_name, type='Padding',
                    bottom=[input_name], top=[output_name])
        layer.pad_param(padding_shapes, padding_values)
        self.caffe_model.add_layer(layer)

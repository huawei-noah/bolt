...
class BertOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        ...
        hidden_states = self.dense(hidden_states)
        # time2 = time.time()
        # print('{}'.format((time2-time1)*1000.0))
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
        return hidden_states
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        _, self.pooled_output = self.bert(
            input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False
        )
        self.pooled_output.retain_grad()
        pooled_output = self.dropout(self.pooled_output)
        ...
        if labels is not None:
            ...
            self.sm = torch.nn.functional.log_softmax(
                self.logits.view(-1, self.num_labels), 1
            )
            self.sm.retain_grad()
            self.loss = torch.nn.functional.nll_loss(
                self.sm, labels.view(-1), None, None, -100, None, "mean"
            )
            return self.loss
        else:
            return self.logits

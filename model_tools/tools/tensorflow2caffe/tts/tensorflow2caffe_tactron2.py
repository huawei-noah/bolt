...
class Tensorflow2CaffeTactron2(Tensorflow2Caffe):
    ...
    class Parameters:
        def __init__(self):
            ...
            self.outputs_per_step = 3 #number of frames to generate at each decoding step (increase to speed up computation and allows for higher batch size, decreases G&L audio quality)
            ...
            self.decoder_query_layers = 1 #number of decoder query lstm lstm layers
            self.decoder_layers = 2 #number of decoder lstm layers
    ...
    def EncoderRNN(self, inputs, size=256, zoneout=0.1, scope_id=0, scope="encoder_LSTM", output_name_prefix=""):
        ...
        outputs = self.extract_lstm(inputs, None, lstm_output_name,
            scope_id+2, steps=-2, scope_name=["fw/encoder_fw_LSTM", "bw/encoder_bw_LSTM"],
            zoneoutCell=zoneout, zoneoutOutput=zoneout)
        return outputs
    ...
    def DecoderRNN(self, inputs, states, layers=2, size=1024, zoneout=0.1, scope_id=0, scope="decoder_rnn", output_name_prefix=""):
        ...
        for i in range(layers):
            ...
            x = self.extract_lstm(x, states[i], lstm_output_name,
                scope_id=scope_id+2, scope_name='cell_{}/decoder_LSTM_{}'.format(i, i+1),
                zoneoutCell=zoneout,
                zoneoutOutput=zoneout)
        return x
    ...
    def generate_encoder(self, inputs=None):
        ...
        if (hp.emotion_dim > 0):
            emotion_input_name = "tts_emotions"
            emotion_input_shape = [self.batch, hp.max_sequence_length]
            self.add_input(emotion_input_name, emotion_input_shape)
        ...
        if (hp.emotion_dim > 0):
            emotion_embedding_inputs = "tts_emotion_embedding"
            self.extract_embedding(emotion_input_name, 2, "emotion_embedding_table", emotion_embedding_inputs)
            if (hp.emotion_dim != self.get_tensor_shape(emotion_embedding_inputs)[2]):
                print("[ERROR] speaker embedding dim emotion_dim(%d) is set to %d" % 
                    (self.get_tensor_shape(emotion_embedding_inputs)[2], hp.emotion_dim))
                exit(1)
        ...
        if (hp.emotion_dim > 0):
            rnn_result = self.add_concat([rnn_result, emotion_embedding_inputs], "encoder_concat", axis=2)
        if (self.params.streaming):
    ...
    def generate_decoder(self, rnn_result, inputs=None):
        ...
        rnn_result_dim = hp.encoder_lstm_units * 2 + hp.emotion_dim
        # TODO rn_result shape
        ...
        if (hp.decoder_query_layers > 0):
            decoder_query_lstm_states = self.prepare_decoder_states(hp.decoder_query_layers, hp.decoder_lstm_units*2, "decoder_query")
        decoder_lstm_states = self.prepare_decoder_states(hp.decoder_layers, hp.decoder_lstm_units*2, "decoder_lstm")
        self.set_input(inputs)
        self.save_input()
        ...
        for i in range(hp.max_iters):
            ...
            if (hp.decoder_query_layers > 0):
                LSTM_output = self.DecoderRNN(LSTM_input, decoder_query_lstm_states, layers=hp.decoder_query_layers,
                ...
                LSTM_output = self.DecoderRNN(LSTM_input, decoder_lstm_states, layers=hp.decoder_layers,
                            size=hp.decoder_lstm_units, zoneout=hp.tacotron_zoneout_rate,
                            scope_id=3, scope='decoder_LSTM', output_name_prefix="decoder_lstm")
            ...
            if (hp.decoder_query_layers > 0):
                multi_rnn_output = self.DecoderRNN(projections_input, decoder_lstm_states, layers=hp.decoder_layers,
                            size=hp.decoder_lstm_units, zoneout=hp.tacotron_zoneout_rate,
                            scope_id=3, scope='decoder_LSTM', output_name_prefix="decoder_lstm")
            else:
                multi_rnn_output = projections_input
            ...
            stop_projection = self.StopProjection(multi_rnn_output, shape=hp.outputs_per_step,

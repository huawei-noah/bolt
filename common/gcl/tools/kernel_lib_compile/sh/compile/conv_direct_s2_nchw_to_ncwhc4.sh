    ...
    do
        if [ "${file##*.}"x = "cl"x ];then
            if [[ "${file}" == "conv_direct_s2_nchw_to_ncwhc4.cl" ]];then
                 ...
                 echo ./gcl_binary --input=$file --output=${file%.*}_75.bin --options=\"${copt}      -D F=7 -D ON=5 -D Fsq=49 -DUSE_HALF\"
                 ...
                 echo ./gcl_binary --input=$file --output=${file%.*}_relu_75.bin --options=\"${copt} -D F=7 -D ON=5 -D Fsq=49 -DUSE_HALF -DUSE_RELU\"
                 echo ./gcl_binary --input=$file --output=${file%.*}_relu6_18.bin --options=\"${copt} -D F=1 -D ON=8 -D Fsq=1  -DUSE_HALF -DUSE_RELU6\"
                 echo ./gcl_binary --input=$file --output=${file%.*}_relu6_37.bin --options=\"${copt} -D F=3 -D ON=7 -D Fsq=9  -DUSE_HALF -DUSE_RELU6\"
                 echo ./gcl_binary --input=$file --output=${file%.*}_relu6_56.bin --options=\"${copt} -D F=5 -D ON=6 -D Fsq=25 -DUSE_HALF -DUSE_RELU6\"
                 echo ./gcl_binary --input=$file --output=${file%.*}_relu6_75.bin --options=\"${copt} -D F=7 -D ON=5 -D Fsq=49 -DUSE_HALF -DUSE_RELU6\"
            fi
        fi
    done

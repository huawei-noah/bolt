    ...
    do
        if [ "${file##*.}"x = "cl"x ];then
            if [[ "${file}" == "conv_direct_spe_fwhs1.cl" ]];then
                ...
                echo ./gcl_binary --input=$file --output=${file%.*}_nobias_4.bin  --options=\"${copt} -D OC=4  -D NO_BIAS\"
                echo ./gcl_binary --input=$file --output=${file%.*}_nobias_8.bin  --options=\"${copt} -D OC=8  -D NO_BIAS\"
                echo ./gcl_binary --input=$file --output=${file%.*}_nobias_16.bin --options=\"${copt} -D OC=16 -D NO_BIAS\"
                ...
                echo ./gcl_binary --input=$file --output=${file%.*}_relu6_4.bin  --options=\"${copt} -D OC=4  -D USE_RELU6\"
                echo ./gcl_binary --input=$file --output=${file%.*}_relu6_8.bin  --options=\"${copt} -D OC=8  -D USE_RELU6\"
                echo ./gcl_binary --input=$file --output=${file%.*}_relu6_16.bin --options=\"${copt} -D OC=16 -D USE_RELU6\"
            fi
        fi
    done
